...
class gopigo_control:
    ...
    def move(self,stat,screen):
        screen.move(1,0)
        screen.clrtoeol()
        screen.addstr(1,0,"gpg_mode:"+stat.gpg_mode)
        screen.move(2,0)
        screen.clrtoeol()
        screen.addstr(2,0,"drive_degree:"+str(stat.gpg_drive_degree))
        screen.move(3,0)
        screen.clrtoeol()
        screen.addstr(3,0,"turn_degree:"+str(self.calc_gopigo_degree(stat.gpg_turn_degree)))
        if stat.gpg_mode == "drive":
            self.pi.drive_degrees(stat.gpg_drive_degree,blocking=False)
        elif stat.gpg_mode == "turn":
            self.pi.turn_degrees(stat.gpg_turn_degree,blocking=False)
